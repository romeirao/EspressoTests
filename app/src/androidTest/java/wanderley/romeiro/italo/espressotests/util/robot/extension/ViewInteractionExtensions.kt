...
// TEXT
fun ViewInteraction.checkText(
    msgStringId: Int,
    vararg extraContent: String,
    ...
    check(
        matches(
            allOf(
                isDisplayed(),
                withTextAndParameters(
                    resourceId = msgStringId,
...
// EDIT TEXT
fun ViewInteraction.typeText(text: String, isScrolling: Boolean = false): ViewInteraction =
    if (isScrolling) {
        perform(
            closeSoftKeyboard(),
            scrollTo(),
        ...
        perform(closeSoftKeyboard(), ViewActions.typeText(text), closeSoftKeyboard())
    }

fun ViewInteraction.replaceText(text: String, isScrolling: Boolean = false): ViewInteraction =
    if (isScrolling) {
        perform(
            closeSoftKeyboard(),
            scrollTo(),
        ...
        perform(closeSoftKeyboard(), ViewActions.replaceText(text), closeSoftKeyboard())
    }

// BUTTON
fun ViewInteraction.checkButtonData(
    isEnabled: Boolean,
    msgStringId: Int,
    vararg extraContent: String
